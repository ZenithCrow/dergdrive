...
pub fn init(conf: Conf, gpa: std.mem.Allocator, io: std.Io) InitError!GlobCtx {
    ...
    const priv_key = conf.root_conf.getConf(conf.public_sign_key, gpa, io) catch |err| {
        switch (err) {
    ...
    const pk = .fromBytes(pub_key[0..crypt.SignAlgo.PublicKey.encoded_length].*) catch {
        log.err(non_canon_err_msg_base, .{ "Public", conf.public_sign_key });
        return InitError.GlobCtxInitFailed;
    };

    const sk = .fromBytes(priv_key[0..crypt.SignAlgo.SecretKey.encoded_length].*) catch {
